...
How to start quickly:
 - create db by cmd: source abstract-path/init-sql/schema.sql
 - generate java, mapperFile, mapperXml by cmd: mvn mybatis-generator:generate
 - generate json service interfaces  by yourself, then start boot application by cmd: mvn spring-boot:run
```
Custom Mybatis Generator:
 - generate java entity(class): attributes, setter/getter, customAttributes, comment, annotation
 - generate mapper interface:  customMethods, annotation
 - generate mapper xml for sql.
```
| Name          			| Description           								|
| ------------- 			|---------------------									|
