...
import com.crab.mapper.*;
import com.crab.common.SystemException;
...
public class AclImpl implements IAcl {
	...
	public boolean hasPermission(Integer userId, Integer resourceId) {
		return hasPermission(userId, resourceId, Constant.READ);
	}

	@Override
	public boolean hasPermission(Integer userId, Integer resourceId, Integer permission) {
		//查找直接授予用户的授权
		Acl acl = findACL(Constant.TYPE_USER, userId, resourceId);
		if (acl != null) {
			int yesOrNo = Constant.getPermission(acl.getAclState(), acl.getAclExtState(), permission);
			
			//如果是确定的授权
			if(yesOrNo != Constant.ACL_NEUTRAL){
				return yesOrNo == Constant.ACL_YES ? true : false;
			}
		}
		// 查找用户的角色授权
		List<Integer> roles = roleMapper.getRoleIdsByUserId(userId);
		for (Integer rid : roles) {
			acl = findACL(Constant.TYPE_ROLE, rid, resourceId);
			
			if(null != acl)
				return Constant.getPermission(acl.getAclState(), acl.getAclExtState(), permission) == Constant.ACL_YES ? true : false;
		}
		return false;
	}
	...
	public List<Module> searchModules(Integer userId, Integer modulePid) {
		...
		for (Integer rid : roles) {
			// 查找角色所拥有的访问列表
			List<Acl> acls = aclMapper.getAcls(Constant.TYPE_ROLE, rid);
		...
		// 查找授予用户的不继承授权列表
		List<Acl> acls= aclMapper.getAclsNotExtends(Constant.TYPE_USER, userId);
		for (Acl acl : acls) {
			temp.put(acl.getResourceSn(), acl);
		}
		...
		for (Entry<Integer, Acl> entry : entities) {
			...
			if (Constant.ACL_NO == Constant.getPermission(acl.getAclState(), acl.getAclExtState(), Constant.READ)) {
				delResources.add(entry.getKey());
			}
		}
		
		// 查找授予用户的继承授权列表
		acls = aclMapper.getAclsExtends(Constant.TYPE_USER, userId);
	...
	public Map<String, Boolean> searchModulePermission(Integer userId, Integer resourceSn) {
		Acl acl = aclMapper.searchModulePermission(Constant.TYPE_USER, userId, resourceSn);
		if (acl == null) {
			List<Integer> roleIds = roleMapper.getRoleIdsByUserId(userId);
			for (Integer sn : roleIds) {
				acl = aclMapper.searchModulePermission(Constant.TYPE_ROLE, sn, resourceSn);
				if (acl != null) break;
			}
		}
		...
		for (Permission permission : permissionList) {
			resultMap.put(permission.getName(), Constant.getPermission(acl.getAclState(), acl.getAclExtState(), permission.getSn()) == Constant.ACL_YES ? true : false);
		}
		return resultMap;
	}
	...
	public void updatePermission(Integer moduleId, Integer userId, String permission, Boolean yesno) {
		Acl acl = findACL(Constant.TYPE_ROLE, userId, moduleId);
		Permission permi = permissionMapper.findByName(permission);
		Constant.setPermission(acl.getAclState(), permi.getSn(), yesno);
